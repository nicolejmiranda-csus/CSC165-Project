...
public class GameServerUDP extends GameConnectionServer<UUID> 
{
	...
	public void processPacket(Object o, InetAddress senderIP, int senderPort)
	{
		...
		{	// JOIN -- Case where client just joined the server
			...
			// Received Message Format: (create,localId,x,y,z,avatarName)
			if(messageTokens[0].compareTo("create") == 0)
			{	UUID clientID = UUID.fromString(messageTokens[1]);
				String[] pos = {messageTokens[2], messageTokens[3], messageTokens[4]};
				String avatarName = (messageTokens.length > 5) ? messageTokens[5] : "boy";
				sendCreateMessages(clientID, pos, avatarName);
				sendWantsDetailsMessages(clientID);
			}
			
			// DETAILS-FOR --- Case where server receives a details for message
			// Received Message Format: (dsfr,senderId,targetId,x,y,z,avatarName)
			...
			{	UUID clientID = UUID.fromString(messageTokens[1]);
				...
				String avatarName = (messageTokens.length > 6) ? messageTokens[6] : "boy";
				sendDetailsForMessage(remoteID, clientID, pos, avatarName);
			}
	...
	{	try 
		{	System.out.println("trying to confirm join");
			...
			System.out.println("join confirmation sent to - " + clientID.toString());
		} 
		catch (IOException e) 
		{	e.printStackTrace();
	}	}
	...
	{	try 
		{	String message = new String("bye," + clientID.toString());
			...
			System.out.println("bye message sent to client " + clientID.toString());
		} 
		catch (IOException e) 
		{	e.printStackTrace();
	}	}
	...
	public void sendCreateMessages(UUID clientID, String[] position, String avatarName)
	{	try
		{	String message = new String("create," + clientID.toString());
			...
			message += "," + avatarName;
			forwardPacketToAll(message, clientID);
			System.out.println("creation of " + avatarName + " message sent to all clients for client " + clientID.toString());
		}
		catch (IOException e)
		{	e.printStackTrace();
	}	}
	...
	public void sendDetailsForMessage(UUID clientID, UUID remoteId, String[] position, String avatarName)
	{	try
		{	String message = new String("dsfr," + remoteId.toString());
			...
			message += "," + avatarName;
			sendPacket(message, clientID);
			System.out.println("details for " + avatarName + " message sent to client " + clientID.toString() + " for remote client " + remoteId.toString());
		}
		catch (IOException e)
		{	e.printStackTrace();
	}	}
	...
	{	try 
		{	String message = new String("wsds," + clientID.toString());	
			forwardPacketToAll(message, clientID);
			System.out.println("wants details message sent to all clients for client " + clientID.toString());
		} 
		catch (IOException e) 
		{	e.printStackTrace();
	}	}
	...
	{	try 
		{	String message = new String("move," + clientID.toString());
			...
			message += "," + position[2];
			forwardPacketToAll(message, clientID);
			System.out.println("move message sent to all clients for client " + clientID.toString());
