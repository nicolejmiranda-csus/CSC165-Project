...
public class GameObject
{
	...
	/**
	 * Rotates this GameObject around its local Y axis (local-space yaw).
	 */
	public void yaw(float radians)
	{
		Matrix4f r = new Matrix4f(localRotation);
		r.mul(new Matrix4f().rotationY(radians));
		setLocalRotation(r);
	}

	/**
	 * Rotates this GameObject around its local X axis (local-space pitch).
	 */
	public void pitch(float radians)
	{
		Matrix4f r = new Matrix4f(localRotation);
		r.mul(new Matrix4f().rotationX(radians));
		setLocalRotation(r);
	}

	/**
	 * Rotates this GameObject around the world Y axis.
	 * This differs from {@link #yaw(float)} because the yaw rotation is applied in world space
	 * instead of the object local space.
	 */
	public void globalYaw(float radians)
	{
		// "Global yaw" means rotate around WORLD Y-axis, not the object's current up axis.
		// Pre-multiply the yaw so the rotation is applied in world coordinates.
		Matrix4f cur = getLocalRotation();
		Matrix4f yaw = (new Matrix4f()).rotationY(radians);
		Matrix4f out = (new Matrix4f());
		yaw.mul(cur, out); // out = yaw * cur
